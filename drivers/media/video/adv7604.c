...
enum adv76xx_type {//设备类型，兼容两种芯片
	ADV7604,
	ADV7611,
};

struct adv76xx_reg_seq {//寄存器值数据对
	unsigned int reg;
	u8 val;
};

struct adv76xx_format_info {//格式信息
	u32 code;//编码
	u8 op_ch_sel;//通道选择
	bool rgb_out;//输出
	bool swap_cb_cr;//是否交换cb和cr
	u8 op_format_sel;//格式的选择
};
...
struct adv76xx_chip_info {//芯片信息上下文
	...
	unsigned int edid_enable_reg;//edid使能寄存器
	unsigned int edid_status_reg;//edid状态寄存器
	unsigned int lcf_reg;

	unsigned int cable_det_mask;//电缆线探测掩码
	unsigned int tdms_lock_mask;
	unsigned int fmt_change_digital_mask;
	unsigned int cp_csc;

	const struct adv76xx_format_info *formats;//视频格式信息
	...
	/* 0 = AFE, 1 = HDMI */
	const struct adv76xx_reg_seq *recommended_settings[2];//推荐设置
	unsigned int num_recommended_settings[2];//推荐设置的数量
...
struct adv76xx_state {//芯片的状态上下文，为驱动的上下文环境
	const struct adv76xx_chip_info *info;//芯片信息
	struct adv76xx_platform_data pdata;//平台驱动附加数据
	...
	enum adv76xx_pad selected_input;//选择的输入通道

	struct v4l2_dv_timings timings;//使用的视频格式的时钟
	const struct adv76xx_format_info *format;//视频格式
...
static const struct v4l2_dv_timings adv76xx_timings[] = {
	V4L2_DV_BT_CEA_720X480P59_94,//CEA规范的一些时钟格式，分别为480p,576p,720p和1080p
	...
	/* sorted by DMT ID *///DMT规范的一些时钟格式，一般的PC格式
	V4L2_DV_BT_DMT_640X350P85,
	V4L2_DV_BT_DMT_640X400P85,
	V4L2_DV_BT_DMT_720X400P85,
	V4L2_DV_BT_DMT_640X480P60,//640x480
	V4L2_DV_BT_DMT_640X480P72,
	V4L2_DV_BT_DMT_640X480P75,
	V4L2_DV_BT_DMT_640X480P85,
	V4L2_DV_BT_DMT_800X600P56,//800x600
	V4L2_DV_BT_DMT_800X600P60,
	V4L2_DV_BT_DMT_800X600P72,
	V4L2_DV_BT_DMT_800X600P75,
	V4L2_DV_BT_DMT_800X600P85,
	V4L2_DV_BT_DMT_848X480P60,//MAC的格式
	V4L2_DV_BT_DMT_1024X768P60,//1024x768
	...
	V4L2_DV_BT_DMT_1360X768P60,//1360x768
	...
	V4L2_DV_BT_DMT_1366X768P60_RB,//1366x768
	V4L2_DV_BT_DMT_1366X768P60,
	V4L2_DV_BT_DMT_1920X1080P60,//1920x1080
	{ },
};

struct adv76xx_video_standards {//视频格式标准
	struct v4l2_dv_timings timings;//标准时序
	u8 vid_std;//寄存器的值
	u8 v_freq;//频率的值
};

/* sorted by number of lines */
static const struct adv76xx_video_standards adv7604_prim_mode_comp[] = {//comp格式预设的格式，使用这种格式，两个寄存器应该配置的值
	...
	{ V4L2_DV_BT_CEA_1920X1080P60, 0x1e, 0x00 },//1080p60，vid-std=0x1e,v_freq=0x00
	/* TODO add 1920x1080P60_RB (CVT timing) */
	{ },
};

/* sorted by number of lines */
static const struct adv76xx_video_standards adv7604_prim_mode_gr[] = {//graph格式预设的格式，两个寄存器的对应值
...
/* sorted by number of lines */
static const struct adv76xx_video_standards adv76xx_prim_mode_hdmi_comp[] = {//hdmi_comp时的寄存器配置
	...
	{ V4L2_DV_BT_CEA_1920X1080P30, 0x1e, 0x02 },
	{ V4L2_DV_BT_CEA_1920X1080P50, 0x1e, 0x01 },
	{ V4L2_DV_BT_CEA_1920X1080P60, 0x1e, 0x00 },//1920x1080p60，使用的值为0x1e和0x00
	{ },
};

/* sorted by number of lines */
static const struct adv76xx_video_standards adv76xx_prim_mode_hdmi_gr[] = {//hdmi_graph时的寄存器配置
	{ V4L2_DV_BT_DMT_640X480P60, 0x08, 0x00 },
	{ V4L2_DV_BT_DMT_640X480P72, 0x09, 0x00 },
	{ V4L2_DV_BT_DMT_640X480P75, 0x0a, 0x00 },
	{ V4L2_DV_BT_DMT_640X480P85, 0x0b, 0x00 },
...
static inline struct adv76xx_state *to_state(struct v4l2_subdev *sd)//由v4l2设备指针找到驱动上下文
{
	return container_of(sd, struct adv76xx_state, sd);
}

static inline unsigned htotal(const struct v4l2_bt_timings *t)//根据视频格式，算出帧对应的帧的宽度和高度
{
...
static s32 adv_smbus_read_byte_data(struct adv76xx_state *state,//使用SMbus接口封装函数读取寄存器的地址，芯片不同功能部分拥有不同的i2c地址
...
static s32 adv_smbus_write_byte_data(struct adv76xx_state *state,//使用smbus接口函数写入寄存器一个值
				     enum adv76xx_page page, u8 command,
				     u8 value)
{
	struct i2c_client *client = state->i2c_clients[page];//不同的功能区对应的i2c的地址不同
	union i2c_smbus_data data;
	int err;
	int i;
...
static s32 adv_smbus_write_i2c_block_data(struct adv76xx_state *state,
					  enum adv76xx_page page, u8 command,
					  unsigned length, const u8 *values)//写入一块连续的数据，使用寄存器偏移的形式
{
...
static inline int io_read(struct v4l2_subdev *sd, u8 reg)//io读和io写的封装
{
...
static inline int io_write_clr_set(struct v4l2_subdev *sd, u8 reg, u8 mask, u8 val)//读出后清零
{
	return io_write(sd, reg, (io_read(sd, reg) & ~mask) | val);
}

static inline int avlink_read(struct v4l2_subdev *sd, u8 reg)//avlink功能区读写实现
{
	struct adv76xx_state *state = to_state(sd);
...
static inline int cec_read(struct v4l2_subdev *sd, u8 reg)//cec功能区的读写实现
{
	struct adv76xx_state *state = to_state(sd);
...
static inline int infoframe_read(struct v4l2_subdev *sd, u8 reg)//infoframe功能区的读写实现
{
	struct adv76xx_state *state = to_state(sd);
...
static inline int afe_read(struct v4l2_subdev *sd, u8 reg)//afe功能区的读写实现
{
	struct adv76xx_state *state = to_state(sd);
...
static inline int rep_read(struct v4l2_subdev *sd, u8 reg)//repeat功能区的读写实现
{
	struct adv76xx_state *state = to_state(sd);
...
static inline int rep_write_clr_set(struct v4l2_subdev *sd, u8 reg, u8 mask, u8 val)//清除具体的末尾的操作写
{
	return rep_write(sd, reg, (rep_read(sd, reg) & ~mask) | val);
}

static inline int edid_read(struct v4l2_subdev *sd, u8 reg)//edid功能区的读写实现
{
	struct adv76xx_state *state = to_state(sd);
...
static inline int edid_write_block(struct v4l2_subdev *sd,//填充edid的块写功能的实现
...
static void adv76xx_set_hpd(struct adv76xx_state *state, unsigned int hpd)//hpd热插拔信号的手动控制
{
	unsigned int i;
...
static void adv76xx_delayed_work_enable_hotplug(struct work_struct *work)//封装后的使能hpd热插拔函数
{
...
static inline int hdmi_read(struct v4l2_subdev *sd, u8 reg)//hdmi功能区的读写实现
{
	struct adv76xx_state *state = to_state(sd);
...
static inline int hdmi_write_clr_set(struct v4l2_subdev *sd, u8 reg, u8 mask, u8 val)//更改某些寄存器位的操作
{
	return hdmi_write(sd, reg, (hdmi_read(sd, reg) & ~mask) | val);
}

static inline int test_write(struct v4l2_subdev *sd, u8 reg, u8 val)//test功能区的写操作
{
	struct adv76xx_state *state = to_state(sd);
...
static inline int cp_read(struct v4l2_subdev *sd, u8 reg)//cp功能区的读写操作实现
{
	struct adv76xx_state *state = to_state(sd);
...
static inline int vdp_read(struct v4l2_subdev *sd, u8 reg)//vdp功能区的读写实现
{
	struct adv76xx_state *state = to_state(sd);
...
#define ADV76XX_REG(page, offset)	(((page) << 8) | (offset))//寄存器的地址为8bit,这里使用16bit代表唯一的寄存器地址，高8bit为功能区地址
#define ADV76XX_REG_SEQ_TERM		0xffff

#ifdef CONFIG_VIDEO_ADV_DEBUG//adv芯片的测试功能，主要检查寄存器地址是否合法，读取寄存器的值
...
static int adv76xx_write_reg(struct v4l2_subdev *sd, unsigned int reg, u8 val)//大的封装函数，寄存器的写和读，依次传入16bit寄存器地址和值
{
	struct adv76xx_state *state = to_state(sd);
	unsigned int page = reg >> 8;
...
static void adv76xx_write_reg_seq(struct v4l2_subdev *sd,//写序列，从结构体中依次读取寄存器地址和写入的值
...
static const struct adv76xx_format_info adv7604_formats[] = {
	{ MEDIA_BUS_FMT_RGB888_1X24,//24位线宽，RGB各8线
	ADV76XX_OP_CH_SEL_RGB,//选择RGB模式
	true, //rgb输出
	false,//不交换cb和cr
	ADV76XX_OP_MODE_SEL_SDR_444 | ADV76XX_OP_FORMAT_SEL_8BIT//格式标准为SDR_444,格式选择8bit宽
	},//我们关系RGB888的格式，每个分量为8bit带宽
	...
	{ MEDIA_BUS_FMT_UYVY10_1X20, ADV76XX_OP_CH_SEL_RBG, false, false,
	  ADV76XX_OP_MODE_SEL_SDR_422_2X | ADV7604_OP_FORMAT_SEL_10BIT },//确认8bit和10bit代表的含义差异，难度是adc的位宽？
	...
	{ MEDIA_BUS_FMT_UYVY12_1X24, ADV76XX_OP_CH_SEL_RBG, false, false,
	  ADV76XX_OP_MODE_SEL_SDR_422_2X | ADV76XX_OP_FORMAT_SEL_12BIT },//还有12bit，看来adc的精度值不是随意设置的，待确认
...
static const struct adv76xx_format_info adv7611_formats[] = {
	{ 
		MEDIA_BUS_FMT_RGB888_1X24, //24位宽，各8bit
		ADV76XX_OP_CH_SEL_RGB,//使用RGB格式
		true,//使用rgb输出
		false,//不交换cb和cr
		ADV76XX_OP_MODE_SEL_SDR_444 | ADV76XX_OP_FORMAT_SEL_8BIT //使用SDR_444,adc的精度要求为8bit
		},
	{ //上电后的默认视频格式
...
adv76xx_format_info(struct adv76xx_state *state, u32 code)//由格式码找到具体的信息
{
	unsigned int i;
...
static inline bool is_analog_input(struct v4l2_subdev *sd)//是否为数字输入还是模拟输入
{
	struct adv76xx_state *state = to_state(sd);
...
static inline bool is_digital_input(struct v4l2_subdev *sd)//adv7611支持AB两路，adv7611支持ABCD四路
{
	struct adv76xx_state *state = to_state(sd);
...
static unsigned int adv7604_read_cable_det(struct v4l2_subdev *sd)//看是否线缆已经接入
{
...
static int adv76xx_s_detect_tx_5v_ctrl(struct v4l2_subdev *sd)//v4l2的接口要求，是否设置电源为5v
{
	struct adv76xx_state *state = to_state(sd);
...
static int find_and_set_predefined_video_timings(struct v4l2_subdev *sd,//v4l2的接口要求，查询设备的预设的视频格式
		u8 prim_mode,
		...
		const struct v4l2_dv_timings *timings)
{
	...
	for (i = 0; predef_vid_timings[i].timings.bt.width; i++) {
		if (!v4l2_match_dv_timings(timings, &predef_vid_timings[i].timings,
			...
			continue;
		io_write(sd, 0x00, predef_vid_timings[i].vid_std); /* video std *///参数的含义依次为page页，寄存器偏移，值
		io_write(sd, 0x01, (predef_vid_timings[i].v_freq << 4) +
				prim_mode); /* v_freq and prim mode *///核对
		return 0;
	}
...
static int configure_predefined_video_timings(struct v4l2_subdev *sd,
		struct v4l2_dv_timings *timings)//配置预设的时序要求
{
	struct adv76xx_state *state = to_state(sd);
	int err;
...
static void configure_custom_video_timings(struct v4l2_subdev *sd,
		const struct v4l2_bt_timings *bt)//客制化的视频格式
{
	struct adv76xx_state *state = to_state(sd);
	u32 width = htotal(bt);
...
static void set_rgb_quantization_range(struct v4l2_subdev *sd)
{
	...
	case V4L2_DV_RGB_RANGE_AUTO:
		...
		if (hdmi_signal) {
			 ...
			 * Set automode */
			io_write_clr_set(sd, 0x02, 0xf0, 0xf0);//HDMI信号的值
			break;
		}
...
static int adv76xx_s_ctrl(struct v4l2_ctrl *ctrl)//v4l2的接口实现set功能的实现
{
	...
	switch (ctrl->id) {
	case V4L2_CID_BRIGHTNESS://调节亮度
		cp_write(sd, 0x3c, ctrl->val);
		return 0;
	case V4L2_CID_CONTRAST://对比度
		cp_write(sd, 0x3a, ctrl->val);
		return 0;
	case V4L2_CID_SATURATION://色度
		cp_write(sd, 0x3b, ctrl->val);
		return 0;
	case V4L2_CID_HUE://灰度
		cp_write(sd, 0x3d, ctrl->val);
		return 0;
	case  V4L2_CID_DV_RX_RGB_RANGE://RGB色彩的范围
		state->rgb_quantization_range = ctrl->val;
		set_rgb_quantization_range(sd);
		return 0;
	case V4L2_CID_ADV_RX_ANALOG_SAMPLING_PHASE://模拟采样解析
		if (!adv76xx_has_afe(state))
			return -EINVAL;
		...
		return 0;
	case V4L2_CID_ADV_RX_FREE_RUN_COLOR_MANUAL://free run模式使能
	...
	case V4L2_CID_ADV_RX_FREE_RUN_COLOR:
		...
		return 0;
	case V4L2_CID_ADV_RX_FREE_RUN_MODE://freerun模式的配置
		switch (ctrl->val) {
		case 0: /* Disabled *///关闭
		...
		case 1: /* Enabled *///使能
			...
			cp_write_clr_set(sd, 0xba, 0x1, 1);//使能FREERUN需要设置的两个寄存器
...
static inline bool no_lock_tmds(struct v4l2_subdev *sd)//非锁定的tmds
{
	struct adv76xx_state *state = to_state(sd);
	const struct adv76xx_chip_info *info = state->info;
...
static inline bool is_hdmi(struct v4l2_subdev *sd)//是否为HDMI信号
{
...
static inline bool no_lock_stdi(struct v4l2_subdev *sd)//非锁定stdi
{
	/* TODO channel 2 */
	return !(cp_read(sd, 0xb1) & 0x80);
}

static inline bool no_signal(struct v4l2_subdev *sd)//无信号的检测方法
{
	bool ret;
...
static inline bool no_lock_cp(struct v4l2_subdev *sd)//状态，非锁定cp
{
	struct adv76xx_state *state = to_state(sd);
...
static inline bool in_free_run(struct v4l2_subdev *sd)//是否为freerun模式
{
	return cp_read(sd, 0xff) & 0x10;
}

static int adv76xx_g_input_status(struct v4l2_subdev *sd, u32 *status)//V4L2的接口，输入的状态判断
{
	*status = 0;
...
static int adv76xx_enum_dv_timings(struct v4l2_subdev *sd,//V4L2的接口实现，枚举支持的视频格式
...
static int adv76xx_dv_timings_cap(struct v4l2_subdev *sd,//芯片支持的能力查询，V4L2的接口实现
...
static unsigned int adv7604_read_hdmi_pixelclock(struct v4l2_subdev *sd)//7604读取HDMI视频格式的像素时钟
{
	unsigned int freq;
	int a, b;
...
static unsigned int adv7611_read_hdmi_pixelclock(struct v4l2_subdev *sd)//7611的视频时钟
{
	int a, b;
...
static int adv76xx_query_dv_timings(struct v4l2_subdev *sd,
			struct v4l2_dv_timings *timings)//V4L2的查询时钟功能
{
	struct adv76xx_state *state = to_state(sd);
	const struct adv76xx_chip_info *info = state->info;
	...
	bt->interlaced = stdi.interlaced ?//是隔行还是逐行
	...
	if (is_digital_input(sd)) {
		...
		bt->width = hdmi_read16(sd, 0x07, 0xfff);//视频宽度
		bt->height = hdmi_read16(sd, 0x09, 0xfff);//高度
		bt->pixelclock = info->read_hdmi_pixelclock(sd);//像素时钟
		bt->hfrontporch = hdmi_read16(sd, 0x20, 0x3ff);//水平的前肩
		bt->hsync = hdmi_read16(sd, 0x22, 0x3ff);//水平刷新频率hsync
		bt->hbackporch = hdmi_read16(sd, 0x24, 0x3ff);//水平的后肩
		bt->vfrontporch = hdmi_read16(sd, 0x2a, 0x1fff) / 2;//垂直的前肩
		bt->vsync = hdmi_read16(sd, 0x2e, 0x1fff) / 2;//vsync
		bt->vbackporch = hdmi_read16(sd, 0x32, 0x1fff) / 2;//垂直的后肩
...
found:
	...
	if ((is_analog_input(sd) && bt->pixelclock > 170000000) ||
			(is_digital_input(sd) && bt->pixelclock > 225000000)) {//模拟信号最大到170MHZ，数字信号到225MHZ
...
static int adv76xx_s_dv_timings(struct v4l2_subdev *sd,//V4L2的接口实现
		struct v4l2_dv_timings *timings)
{
	struct adv76xx_state *state = to_state(sd);
...
static void adv7604_set_termination(struct v4l2_subdev *sd, bool enable)//终止
{
...
static void enable_input(struct v4l2_subdev *sd)//使能输入
{
	struct adv76xx_state *state = to_state(sd);
...
static void disable_input(struct v4l2_subdev *sd)//关闭输出
{
	struct adv76xx_state *state = to_state(sd);
...
static void select_input(struct v4l2_subdev *sd)//选择输入
{
	struct adv76xx_state *state = to_state(sd);
	const struct adv76xx_chip_info *info = state->info;
...
static int adv76xx_s_routing(struct v4l2_subdev *sd,
		u32 input, u32 output, u32 config)//切换输入源
{
	struct adv76xx_state *state = to_state(sd);
...
static void adv76xx_setup_format(struct adv76xx_state *state)//V4L2的接口
{
	struct v4l2_subdev *sd = &state->sd;
...
static int adv76xx_get_format(struct v4l2_subdev *sd,
			      struct v4l2_subdev_pad_config *cfg,
			      struct v4l2_subdev_format *format)//V4L2的接口
{
	struct adv76xx_state *state = to_state(sd);
...
static const struct v4l2_subdev_ops adv76xx_ops = {//v4l2子设备的操作函数集合
	.core = &adv76xx_core_ops,//核心操作函数集合
	.video = &adv76xx_video_ops,//视频控制相关的集合
	.pad = &adv76xx_pad_ops,//面板的操作集合
};
...
static const struct adv76xx_chip_info adv76xx_chip_info[] = {
	...
	[ADV7611] = {//关注7611
		.type = ADV7611,
		.has_afe = false,//无afe
		.max_port = ADV76XX_PAD_HDMI_PORT_A,//只支持PortA输入
		.num_dv_ports = 1,//支持一路端口
		.edid_enable_reg = 0x74,//IO:74是否使能内部EDID配置
		.edid_status_reg = 0x76,//状态是IO:76
		.lcf_reg = 0xa3,
		.tdms_lock_mask = 0x43,//tdms的锁定掩码
		.cable_det_mask = 0x01,//线缆探测的掩码
		.fmt_change_digital_mask = 0x01,//数字信号的格式改变的掩码
		.cp_csc = 0xf4,//cp的色彩转换的
		.formats = adv7611_formats,//7611支持的格式
		.nformats = ARRAY_SIZE(adv7611_formats),//格式的个数
		.set_termination = adv7611_set_termination,//设置终端输出
		.setup_irqs = adv7611_setup_irqs,//中断的设置
		.read_hdmi_pixelclock = adv7611_read_hdmi_pixelclock,//读取像素频率
		.read_cable_det = adv7611_read_cable_det,//线缆探测
		.recommended_settings = {
		    [1] = adv7611_recommended_settings_hdmi,//推荐配置信息
		},
		.num_recommended_settings = {//推荐配置的个数
		    [1] = ARRAY_SIZE(adv7611_recommended_settings_hdmi),
		},
		.page_mask = BIT(ADV76XX_PAGE_IO) | BIT(ADV76XX_PAGE_CEC) |//功能区页的值
...
static struct i2c_device_id adv76xx_i2c_id[] = {
	...
	{ "adv7611", (kernel_ulong_t)&adv76xx_chip_info[ADV7611] },//关注7611
	{ }
};
MODULE_DEVICE_TABLE(i2c, adv76xx_i2c_id);

static struct of_device_id adv76xx_of_id[] __maybe_unused = {//设备树的兼容信息
	{ .compatible = "adi,adv7611", .data = &adv76xx_chip_info[ADV7611] },
	{ }
};
MODULE_DEVICE_TABLE(of, adv76xx_of_id);

static int adv76xx_parse_dt(struct adv76xx_state *state)//解析设备树
{
	...
	if (flags & V4L2_MBUS_HSYNC_ACTIVE_HIGH)//视频同步的信号是高电平有效 hsync
		state->pdata.inv_hs_pol = 1;

	if (flags & V4L2_MBUS_VSYNC_ACTIVE_HIGH)//vsync
		state->pdata.inv_vs_pol = 1;

	if (flags & V4L2_MBUS_PCLK_SAMPLE_RISING)
		state->pdata.inv_llc_pol = 1;

	if (bus_cfg.bus_type == V4L2_MBUS_BT656) {//是否使用BT656格式
	...
	/* Use the default I2C addresses. *///配置一些默认的i2c功能区的地址
...
static int adv76xx_reset(struct adv76xx_state *state)//芯片重置功能实现
{
	if (gpio_is_valid(state->reset_gpio)) {
		/* ADV76XX can be reset by a low reset pulse of minimum 5 ms. *///要求中间低电平维持时间至少为5ms，这里设置为10ms时间
		gpio_set_value_cansleep(state->reset_gpio, 0);
		mdelay(10);//5ms临界值可能不好
			 ...
			 const struct i2c_device_id *id)
{
	static const struct v4l2_dv_timings cea640x480 =
		V4L2_DV_BT_CEA_640X480P59_94;//默认的时序640x480p59.94
	struct adv76xx_state *state;//驱动上下文环境
	struct v4l2_ctrl_handler *hdl;//v4l2的处理句柄
	struct v4l2_subdev *sd;//v4l2设备，从功能上划分改设备为v4l2设备
	unsigned int i;
	u16 val;
	int err;
	...
	//默认的I2C的地址为IO地址，且不会改变，0x98或者0x97,末尾有连线决定
	state->i2c_clients[ADV76XX_PAGE_IO] = client;

	/* initialize variables */
	state->restart_stdi_once = true;//重启stdi一次为真
	state->selected_input = ~0;//全1

	if (IS_ENABLED(CONFIG_OF) && client->dev.of_node) {//是否使能是设备树支持
		const struct of_device_id *oid;

		oid = of_match_node(adv76xx_of_id, client->dev.of_node);
		state->info = oid->data;

		err = adv76xx_parse_dt(state);//解析设备树，传入的配置信息
		if (err < 0) {
			v4l_err(client, "DT parsing error\n");
			return err;
		}
	} else if (client->dev.platform_data) {//是否使用平台设备传入数据
		struct adv76xx_platform_data *pdata = client->dev.platform_data;

		state->info = (const struct adv76xx_chip_info *)id->driver_data;//解析平台设备的附带数据
		state->pdata = *pdata;
	} else {//否则提示没有附带数据，找不到设备
		v4l_err(client, "No platform data!\n");
		return -ENODEV;
	}

	/* Request GPIOs. */
	for (i = 0; i < state->info->num_dv_ports; ++i) {//支持的端口号，7611只有1个，这里手动控制HPD信号
		state->hpd_gpio[i] =
	...
	state->reset_gpio = of_get_named_gpio(client->dev.of_node,
								"reset-gpios", 0);//得到重置的GPIO的值
	if (gpio_is_valid(state->reset_gpio)) {
		err = devm_gpio_request_one(&client->dev,
			state->reset_gpio, GPIOF_OUT_INIT_HIGH, "RESET_GPIO");//初始化为高电平
		if (err < 0) {
			v4l2_err(client, "Request reset gpio error\n");
			return err;
		} else {
			adv76xx_reset(state);//重置一下
		}
	}

	state->timings = cea640x480;//默认cea640x480格式
	state->format = adv76xx_format_info(state, MEDIA_BUS_FMT_YUYV8_2X8);//先初始化为这种格式

	sd = &state->sd;//管理v4l2设备
	v4l2_i2c_subdev_init(sd, client, &adv76xx_ops);//初始化v4l2的一些变量，关联一些ops，标准实现
	 ...
	 */
	if (state->info->type == ADV7604) {//7604只能读取到内部的版本号为0x68
	...
	} else {//核实配置信息和芯片是否吻合 只关注7611
		val = (adv_smbus_read_byte_data_check(client, 0xea, false) << 8)//IO:EA 值应该为0x20
		    | (adv_smbus_read_byte_data_check(client, 0xeb, false) << 0);//IO:EB 值应该为0x51
		if (val != 0x2051) {
	...
	/* control handlers */
	hdl = &state->hdl;//句柄
	v4l2_ctrl_handler_init(hdl, adv76xx_has_afe(state) ? 9 : 8);//7611没有afe支持

	v4l2_ctrl_new_std(hdl, &adv76xx_ctrl_ops,//亮度调节范围-128到127，步进为1，默认为0
			V4L2_CID_BRIGHTNESS, -128, 127, 1, 0);
	v4l2_ctrl_new_std(hdl, &adv76xx_ctrl_ops,//对比度0到255，步进1，默认128
			V4L2_CID_CONTRAST, 0, 255, 1, 128);
	v4l2_ctrl_new_std(hdl, &adv76xx_ctrl_ops,
			V4L2_CID_SATURATION, 0, 255, 1, 128);//饱和度，0-255，步进1，默认128
	v4l2_ctrl_new_std(hdl, &adv76xx_ctrl_ops,
			V4L2_CID_HUE, 0, 128, 1, 0);//灰度，0-128，步进1，默认0

	/* private controls */
	state->detect_tx_5v_ctrl = v4l2_ctrl_new_std(hdl, NULL,
			V4L2_CID_DV_RX_POWER_PRESENT, 0,
			(1 << state->info->num_dv_ports) - 1, 0, 0);//是否支持5v电压控制
	state->rgb_quantization_range_ctrl =
		v4l2_ctrl_new_std_menu(hdl, &adv76xx_ctrl_ops,
			V4L2_CID_DV_RX_RGB_RANGE, V4L2_DV_RGB_RANGE_FULL,
			0, V4L2_DV_RGB_RANGE_AUTO);//RGB的色彩表示范围，默认为自动

	/* custom controls */
	if (adv76xx_has_afe(state))//7611不支持afe
...
{
	struct v4l2_subdev *sd = i2c_get_clientdata(client);//得到v4l2设备指针
	struct adv76xx_state *state = to_state(sd);//转变为驱动上下文指针

	cancel_delayed_work(&state->delayed_work_enable_hotplug);//关闭热插拔探测
	destroy_workqueue(state->work_queues);//摧毁工作队列
	v4l2_async_unregister_subdev(sd);//卸载v4l2设备，异步
	media_entity_cleanup(&sd->entity);//清除实体
	adv76xx_unregister_clients(to_state(sd));//卸载客户端
	v4l2_ctrl_handler_free(sd->ctrl_handler);//释放v4l2的句柄
	if (client->irq)
		free_irq(client->irq, state);//释放占用的中断资源
...
static struct i2c_driver adv76xx_driver = {//i2c设备的驱动结构
	.driver = {
		.owner = THIS_MODULE,
		.name = "adv7604",
